...
import { ArrowLeft, CheckCircle2, AlertCircle, Shield, Info } from "lucide-react";

export default function QcCoaViewPage() {
  ...
  if (!currentUser || !coaDoc || !batch || !product) {
    return <div className="p-6"><p className="text-muted-foreground">Loading COA...</p></div>;
  }
  ...
  const signatures = [
    ...
    },
    {
      ...
      name: coaQaSigner?.name || (isPending ? "Pending" : "—"),
      role: "QA Manager",
      dept: "QA",
      date: isIssued ? coaDoc.issuedAt : undefined,
      signed: isIssued,
    },
  ];
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <Button variant="ghost" size="sm" className="mb-4" onClick={() => router.push(`/qc/batch/${batch.id}`)}>
        <ArrowLeft className="w-4 h-4 mr-1" /> Batch Workspace
      </Button>

      {/* Info Banner */}
      <div className={`mb-5 rounded-lg border px-4 py-3 flex items-start gap-3 ${isIssued ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50"}`}>
        {isIssued ? (
        ...
        <div>
          <p className={`text-sm font-semibold ${isIssued ? "text-green-800" : "text-amber-800"}`}>
            {isIssued
              ? "COA Issued — Batch Released"
              : "Awaiting QA Manager Signature"}
          </p>
          <p className={`text-xs mt-0.5 ${isIssued ? "text-green-700" : "text-amber-700"}`}>
            {isIssued
              ? `COA ${coaDoc.docNo} has been signed and issued. Batch ${batch.batchNo} is released.`
              : "This COA was auto-generated from the QA-signed AWS and is pending the QA Manager's final signature for batch release. This is a read-only preview."}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border-2 border-gray-300 rounded-xl shadow-lg relative overflow-hidden">
        ...
        <div className="absolute top-4 right-4 z-10">
          ...
          >
            ...
            {isIssued ? "ISSUED — IMMUTABLE" : "PENDING QA SIGNATURE"}
          </Badge>
        </div>
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                <span className="text-muted-foreground text-xs">Mfg. Date</span>
                <p className="font-medium">{fmtDate(batch.mfgDate)}</p>
              </div>
              <div>
                <span className="text-muted-foreground text-xs">Exp. Date</span>
                <p className="font-medium">{fmtDate(batch.expDate)}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
              <div>
                <span className="text-muted-foreground text-xs">SPEC No.</span>
                <p className="font-mono text-xs">{specDoc?.docNo || "—"}</p>
              </div>
              <div>
                <span className="text-muted-foreground text-xs">A.R. No.</span>
                <p className="font-mono text-xs">{batch.arNo}</p>
              </div>
              <div>
                <span className="text-muted-foreground text-xs">Qty Sampled</span>
                <p className="font-medium">{batch.qtySampled} {batch.qtySampledUom}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            {(coaDoc.coaResults || []).length === 0 ? (
              <p className="text-sm text-muted-foreground italic">No results recorded.</p>
            ) : (
            ...
            >
              ...
              <p
                className={`text-lg font-bold ${
                  coaDoc.complies ? "text-green-800" : "text-red-800"
                }`}
              >
                {coaDoc.complianceStatement}
              </p>
          ...
          {/* Section 5: Authorization / Signature Block */}
          <div>
              ...
              {signatures.map((sig, i) => (
                ...
                >
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">
                    {sig.date ? fmtDateTime(sig.date) : "—"}
                  </p>
                  {sig.signed ? (
                    <CheckCircle2 className="w-4 h-4 text-green-600 mx-auto mt-1" />
                  ) : (
                    <div className="w-4 h-4 rounded-full border-2 border-amber-400 mx-auto mt-1" />
                  )}
                </div>
              ))}
            </div>
      ...
      {isPending && (
        <div className="mt-4 text-center text-xs text-muted-foreground">
          Read-only view for QC. The QA Manager must sign this COA to release the batch.
        </div>
      )}
      ...
      <Card className="mt-6">
        <CardContent className="pt-5">
          ...
          <div className="space-y-2">
            {coaDoc.workflowHistory.length === 0 ? (
              ...
              coaDoc.workflowHistory.map((e, i) => (
                <div key={i} className="flex gap-3 text-xs">
                  <div className="flex flex-col items-center">
                    ...
                    {i < coaDoc.workflowHistory.length - 1 && (
                      <div className="w-px flex-1 bg-gray-200 mt-1" />
                    )}
                  </div>
                  <div className="pb-2">
                    <span className="font-medium">{e.toStatus.replace("_", " ")}</span>
                    {" — "}
                    {e.byUserName}
                    <span className="text-muted-foreground ml-1">{fmtDateTime(e.at)}</span>
                    {e.comment && (
                      <p className="text-muted-foreground italic mt-0.5">&quot;{e.comment}&quot;</p>
                    )}
                  </div>
                </div>
              ))
